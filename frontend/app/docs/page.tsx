...
import { FileText, Upload as UploadIcon, AlertCircle, FileUp } from "lucide-react";
            ...
            {!results || !results.items || results.items.length === 0 ? (
              ...
              <div className="space-y-3">
                {results.items.map(file => (
                  <div key={file.id} className="flex items-center justify-between p-3 bg-zinc-50 border border-zinc-100 rounded-lg hover:bg-zinc-100 transition-colors">
                    <div className="flex items-center gap-3">
                      <FileText className="h-5 w-5 text-blue-500" />
                      <div>
                        <p className="text-sm font-medium text-zinc-900">{file.name}</p>
                        <p className="text-xs text-zinc-500">{file.source} • {(file.size_bytes || 0) / 1000} KB</p>
                      </div>
                    </div>
                    {file.web_url && (
                      <a href={file.web_url} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 hover:underline">
                        Open
                      </a>
                    )}
                  </div>
